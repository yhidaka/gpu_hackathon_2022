    ...
    def sympass4(self, x, fast=1):
        ...
        if self.L != 0:
            ...
            for i in range(self.nkick):
                ...
                x[3] += self._KvLg / (1.0 + x[5])
                x = self._Mb.dot(x)
                x[1] += self._KhLd / (1.0 + x[5])
                x[3] += self._KvLd / (1.0 + x[5])
                x = self._Mb.dot(x)
                x[1] += self._KhLg / (1.0 + x[5])
                x[3] += self._KvLg / (1.0 + x[5])
                x = self._Ma.dot(x)
...
class kmap(drif):
    ...
    def _readkmap(self, fn):
        ...
        return {
            ...
            "y": taby,
            "unit": unit,
            "kx": thetax * self.L / idlen,
            "ky": thetay * self.L / idlen,
        }
    ...
    def sympass4(self, x, fast=1):
        ...
        for m in range(self.nkick):
            if self._kmap1:
                ...
                ky = interp2d(
                    self._kmap1["x"], self._kmap1["y"], self._kmap1["ky"], x[0], x[2]
                )
                x[1] += kx / BRho / self.nkick
                x[3] += ky / BRho / self.nkick
            if self._kmap2:
                kx = interp2d(
                    self._kmap2["x"], self._kmap2["y"], self._kmap2["kx"], x[0], x[2]
                )

                ky = interp2d(
                    self._kmap2["x"], self._kmap2["y"], self._kmap2["ky"], x[0], x[2]
                )
...
class beamline(object):
    ...
    def getElements(
        ...
        * exclude 'exclude'
        ...
        for el in self.bl:
            cond = (
                el.__class__.__name__ in types
    ...
    def pltsigma(self, srange=None, savefn=None, figsize=(15, 5), surflvl=0):
        """
        ...
        for e in ele:
            if e.__class__.__name__ == "drif":
                ...
                s = '{0} = latt.bend("{1}",L={2},angle={3},e1={4},e2={5},K1={6},K2={7},hgap={8},fint={9})\n'.format(
                    e.name, e.name, e.L, e.angle, e.e1, e.e2, e.K1, e.K2, e.hgap, e.fint
                )
            elif e.__class__.__name__ == "quad":
                s = '{0} = latt.quad("{1}",L={2},K1={3},tilt={4})\n'.format(
                    e.name, e.name, e.L, e.K1, e.tilt
                )
            # elif e.__class__.__name__ == 'sole':
            ...
            #        format(e.name,e.name,e.L,e.K1,e.tilt)
                ...
                s = '{0} = latt.aper("{1}",L={2},aper=[{3},{4},{5},{6}])\n'.format(
                    e.name, e.name, e.L, e.aper[0], e.aper[1], e.aper[2], e.aper[3]
                )
            elif e.__class__.__name__ == "kick":
                s = '{0} = latt.kick("{1}",L={2},hkick={3},vkick={4})\n'.format(
                    e.name, e.name, e.L, e.hkick, e.vkick
                )
            elif e.__class__.__name__ == "rfca":
                s = '{0} = latt.rfca("{1}",L={2},voltage={3},freq={4})\n'.format(
                    e.name, e.name, e.L, e.voltage, e.freq
                )
            elif e.__class__.__name__ == "wigg":
                s = '{0} = latt.wigg("{1}",L={2},Bw={3})\n'.format(
                    e.name, e.name, e.L, e.Bw
                )
                ...
                s = '{0} = latt.matr("{1}",L={2},mat={3})\n'.format(
                    e.name, e.name, e.L, m66
                )
            else:
                raise RuntimeError(
                    "unknown element type: {0}".format(e.__class__.__name__)
                )
            fid.write(s)
        fid.write("\n# === Beam Line sequence:\n")
        BL = "BL = ["
...
class cell(beamline):
    ...
    def __init__(self, bl, E=3.0, kcouple=0.01):
        """
    ...
    def _update(self, rad=False, chrom=False, ndt=False, synosc=False, verbose=True):
        """
    ...
    def _period(self):
        ...
        if (
            abs(self.R[0, 0] + self.R[1, 1]) >= 2
            or abs(self.R[2, 2] + self.R[3, 3]) >= 2
        ):
            self._isstable = False
        else:
            mux, self._betax0, self._alfax0, self._gamax0 = matrix2twiss(
                self.R, plane="x"
            )
            muy, self._betay0, self._alfay0, self._gamay0 = matrix2twiss(
                self.R, plane="y"
            )
    ...
    def __repr__(self):
        ...
        if hasattr(self, "nus"):
            s += (
                "Tune: nux = %8.3f, nuy = %8.3f, nus = %8.3f\n"
        ...
        if hasattr(self, "chx0"):
            s += (
                "uncorrected chromaticity: chx0 = %11.3e, chy0 = %11.3e\n"
                % (self.chx0, self.chy0)
                + "\n"
            )
            s += (
                "corrected chromaticity:   chx  = %11.3e, chy  = %11.3e\n"
                % (self.chx, self.chy)
                + "\n"
            )
        if hasattr(self, "emitx"):
            ...
            s += (
                "Radiation damping partition factors: Jx = %11.3e, Jy = %11.3e, Je = %11.3e\n"
                % (self.Jx, self.Jy, self.Je)
                + "\n"
            )
            s += (
                "Radiation damping time [ms]: taux = %11.3e, tauy = %11.3e, taue = %11.3e\n"
    ...
    def rad(self, includeWig=0):
        ...
        for i, elem in enumerate(self.bl):
            if elem.__class__.__name__ == "bend":
                ...
                if K + 1 / R**2 > 0:
                    ...
                    av2 = -K * av_eta / R + (eta0 * t1 + eta2 * t2) / (2.0 * L * R)
                    av_H = (
                        g1 * eta0**2
                        + 2 * a1 * eta0 * etap1
                        + b0 * etap1**2
                        + (2.0 * angle)
                        * (
                            -(g1 * eta0 + a1 * etap1) * (p - S) / (k * p**2)
                            + (a1 * eta0 + b0 * etap1) * (1 - C) / p**2
                        )
                        + angle**2
                        * (
                            g1 * (3.0 * p - 4.0 * S + S * C) / (2.0 * k**2 * p**3)
                            - a1 * (1.0 - C) ** 2 / (k * p**3)
                            + b0 * (p - C * S) / (2.0 * p**3)
                        )
                    )
                ...
                self._I[1] += L / R**2
                self._I[2] += L / abs(R) ** 3
                self._I[3] += L * av_eta / R**3 - 2.0 * L * av2
        ...
        self._emitx = (
            const * lam * grel**2 * (self.I[4] / (self.I[1] - self.I[3])) * 1e9
        )  # nm
    ...
    def findClosedOrbit(
        ...
        for i in range(niter):
            ...
            if (
                abs(x0[0] - x1[0]) <= tol[0]
                and abs(x0[1] - x1[1]) <= tol[1]
                and abs(x0[2] - x1[2]) <= tol[2]
                and abs(x0[3] - x1[3]) <= tol[3]
            ):
                isconvergent = True
                break
            else:
    ...
    def dispersionTrack(
        ...
        if mp:
            ...
            for i, de in enumerate(dE):
                if verbose:
                    sys.stdout.write(
                        "\r %03i out of %03i: dE = %9.4f" % (i + 1, len(dE), de)
                    )
                    sys.stdout.flush()
                xco, xpco, yco, ypco, c, dl = self.findClosedOrbit(
                    fixedenergy=de, sym4=True, niter=100
    ...
    def chromTrack(
        ...
        if tunewindow:
            ...
            rngy = [
                self.nuy - int(self.nuy) - tunewindow,
                self.nuy - int(self.nuy) + tunewindow,
            ]
        else:
            rngx, rngy = [0.0, 1], [0.0, 1]
        if crossHalfInt:
            csm = CSNormalization(
    ...
    def coupledemit(self):
        ...
        for i, ele in enumerate(self.bl):
            if ele.__class__.__name__ != "bend":
                ...
                for si in s:
                    if si == ele.L:
                        fe = bend(
                            L=ele.L, K1=ele.K1, angle=ele.angle, e1=ele.e1, e2=ele.e2
                        )
                    elif si < ele.L:
                        fe = bend(
                ...
                Hx_I += sum(
                    w
                    * (gx_I * dx * dx + 2 * ax_I * dx * dxp + bx_I * dxp * dxp)
                    / ele.R**3
                )
                Hy_I += sum(
                    w
                    * (gy_I * dy * dy + 2 * ay_I * dy * dyp + by_I * dyp * dyp)
                    / ele.R**3
                )
                Hx_II += sum(
                    w
                    * (gx_II * dx * dx + 2 * ax_II * dx * dxp + bx_II * dxp * dxp)
                    / ele.R**3
                )
                Hy_II += sum(
                    w
                    * (gy_II * dy * dy + 2 * ay_II * dy * dyp + by_II * dyp * dyp)
                    / ele.R**3
                )
                I2 += ele.L / ele.R**2
                v1 = ele.tm[:4, :4].dot(v0)
        ...
        self.emit_II = (
            (Hx_II / (I2 - I4_II) + Hy_II / (I2 - I4_II)) * Cq * gamma**2 * 1e9
        )
    ...
    def getf2vsskew(self, index, skews, dk=0.001, verbose=0):
        ...
        for ik, skew in enumerate(skews):
            ...
            if verbose:
                sys.stdout.write(
                    "\r --- %04i out of %04i done (%3i%%) ---"
                    % (ik + 1, nskew, (ik + 1) * 100.0 / nskew)
                )
                sys.stdout.flush()
        self.c1010rm = c1010
        self.c0101rm = c0101
    ...
    def synHam(self, dE, phi, ho=False):
        ...
        if not ho:
            ...
            H = (
                1.0 / 2 * self.h * self.revfreq * self.hoalpha[-2] * dE**2
                + 1.0 / 3 * self.h * self.revfreq * self.hoalpha[-3] * dE**3
        ...
        for i, elem in enumerate(self.bl):
            ...
            if elem.__class__.__name__ == "quad":
                ...
                if elem.K1 == 0.0:
                    dchx = (
                        -(h**2) * sum(w * betax)
                        - 2 * h * sum(w * etap * alphax)
                        + h * sum(w * eta * gammax)
                    )
                    dchy = h * sum(w * eta * gammay)
                else:
                    dchx = (
                        -(elem.K1 + h**2) * sum(w * betax)
    ...
    def simuTbt(self, bpmIndex=[], nturns=1050, x0=[1.0e-3, 0, 0, 0, 0, 0], verbose=0):
        """
        simulate TbT
        """
        if not len(bpmIndex):
    ...
    def geth2(self, **kwargs):
        ...
        h2list = []
        if "h22000" in locals():
            h2list.append(("h22000", h22000 * 1j / 64))
        if "h31000" in locals():
            h2list.append(("h31000", h31000 * 1j / 32))
        if "h11110" in locals():
            h2list.append(("h11110", h11110 * 1j / 16))
        if "h11200" in locals():
            h2list.append(("h11200", h11200 * 1j / 32))
        if "h40000" in locals():
            h2list.append(("h40000", h40000 * 1j / 64))
        if "h20020" in locals():
            h2list.append(("h20020", h20020 * 1j / 64))
        if "h20110" in locals():
            h2list.append(("h20110", h20110 * 1j / 32))
        if "h20200" in locals():
            h2list.append(("h20200", h20200 * 1j / 64))
        if "h00220" in locals():
            h2list.append(("h00220", h00220 * 1j / 64))
        if "h00310" in locals():
            h2list.append(("h00310", h00310 * 1j / 32))
        if "h00400" in locals():
            h2list.append(("h00400", h00400 * 1j / 64))
        if "h21001" in locals():
            h2list.append(("h21001", h21001))
        if "h30001" in locals():
            h2list.append(("h30001", h30001))
        if "h10021" in locals():
            h2list.append(("h10021", h10021))
        if "h10111" in locals():
            h2list.append(("h10111", h10111))
        if "h10201" in locals():
            h2list.append(("h10201", h10201))
        if "h11002" in locals():
            h2list.append(("h11002", h11002))
        if "h20002" in locals():
            h2list.append(("h20002", h20002))
        if "h00112" in locals():
            h2list.append(("h00112", h00112))
        if "h00202" in locals():
            h2list.append(("h00202", h00202))
        if "h10003" in locals():
            h2list.append(("h10003", h10003))
        if "h00004" in locals():
            h2list.append(("h00004", h00004))
    ...
    def getCoupleOrm(self, cindex=None, bindex=None):
        ...
        for ic, ci in enumerate(cindex):
            ...
            for ib, bi in enumerate(bindex):
                ...
                sys.stdout.flush()
                sys.stdout.write(
                    "\rV: cor index: %04i, bpm index: %04i" % (ic + 1, ib + 1)
                )
        self.coupleorm = rm
    ...
    def finddyap(
        self, xmin=-3e-2, xmax=3e-2, ymin=0, ymax=1.5e-2, nx=61, ny=16, dp=0, nturn=512
    ):
        """
        ...
        if dfu:
            ...
            for i in range(nx * ny):
                ...
                if any(ncp.isnan(tbt[:, 0, i])) or any(ncp.isnan(tbt[:, 2, i])):
                    ...
                    if naf:
                        v1x = naff(tbt[: nturn / 2, 0, i], ni=1, rng=rngx, verbose=0)[
                            0
                        ][0]
                        v2x = naff(tbt[nturn / 2 :, 0, i], ni=1, rng=rngx, verbose=0)[
                            0
                        ][0]
                        v1y = naff(tbt[: nturn / 2, 2, i], ni=1, rng=rngy, verbose=0)[
                            0
                        ][0]
                        v2y = naff(tbt[nturn / 2 :, 2, i], ni=1, rng=rngy, verbose=0)[
                            0
                        ][0]
                    else:
                        v1x = getTuneTbT(
                            tbt[: nturn / 2, 0, i], rng=rngx, verbose=0, hann=1
                        )[0]
                        v2x = getTuneTbT(
                            tbt[nturn / 2 :, 0, i], rng=rngx, verbose=0, hann=1
                        )[0]
                        v1y = getTuneTbT(
                            tbt[: nturn / 2, 2, i], rng=rngy, verbose=0, hann=1
                        )[0]
                        v2y = getTuneTbT(
                            tbt[nturn / 2 :, 2, i], rng=rngy, verbose=0, hann=1
                        )[0]
            ...
            for ny in range(-1 * order, order):
                for p in range(nx - order, nx + order):
                    ...
                    if ny != 0:
                        ...
                        ymin1 = (p - nx * xmin1) * 1.0 / ny
                        ymax1 = (p - nx * xmax1) * 1.0 / ny
                    else:
                        xmin1 = p * 1.0 / nx
                        xmax1 = p * 1.0 / nx
    ...
    def pltdyapXD(
        self, xyunit="mm", figsize=(8, 6), mode="contourf", fn=None, fndf=None
    ):
        """
        plot dynamic aperture
        """
        if self.dyapXD:
            ...
            if "dfu" in self.dyapXD.keys():
                ...
                if mode == "contourf":
                    df = plt.contourf(
                        self.dyapXD["dgrid"], self.dyapXD["xgrid"] * unit, maskeddfu, 1
                    )
                else:
                    df = plt.pcolor(
                        self.dyapXD["dgrid"], self.dyapXD["xgrid"] * unit, maskeddfu
                    )
                plt.ylabel("x [%s]" % xyunit)
                plt.xlabel(r"$\delta=\frac{\Delta P}{P}$")
                plt.title("Dynamic aperture for x vs. delta")
...
def twmat(elem, s):
    ...
    """
    if (
        elem.__class__.__name__ == "drif"
        or elem.__class__.__name__ == "sext"
        or elem.__class__.__name__ == "kick"
        or elem.__class__.__name__ == "rfca"
        or elem.__class__.__name__ == "aper"
        or elem.__class__.__name__ == "mult"
    ):
        fe = drif(L=s)
        ...
        if s == elem.L:
            fe = bend(L=elem.L, K1=elem.K1, angle=elem.angle, e1=elem.e1, e2=elem.e2)
        elif s < elem.L:
...
def chkap(x, xap=[-1.0, 1.0], yap=[-1.0, 1.0]):
    """
...
def optm(
    aline, var, con, wgh, xtol=1.0e-8, ftol=1.0e-8, maxiter=1000, maxfun=1000, disp=0
):
    """
...
# --- FFT and NAFF
def pickPeak(xpw, rng=[0.0, 0.5], ith=1):
    """
...
def getTuneTbT(
    xi, rng=[0.0, 0.5], ith=1, hann=False, verbose=False, semilog=False, figsize=(10, 3)
):
    """
...
def CSNormalization(betax, alfax, betay, alfay, combine=True):
    ...
    m = np.mat(
        [
            [1 / sqtbx, 0, 0, 0],
            [alfax / sqtbx, sqtbx, 0, 0],
            [0, 0, 1 / sqtby, 0],
            [0, 0, alfay / sqtby, sqtby],
        ]
    )
    if not combine:
        return m
